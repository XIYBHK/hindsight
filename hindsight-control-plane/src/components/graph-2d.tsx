          ...
          >
            <div className="font-medium capitalize mb-1">
              {(() => {
                ...
                const typeLabel = t(`graph.linkTypes.${type}`, {
                  defaultValue: type.replace("_", " "),
                });
                if (["causes", "caused_by", "enables", "prevents"].includes(type)) {
                  return t("graph.tooltip.causalLink", { type: typeLabel });
                }
                return t("graph.tooltip.linkType", { type: typeLabel });
              })()}
            </div>
            {hoveredLink.entity && (
              <div className="text-xs opacity-80">
                {t("graph.tooltip.entity")}:{" "}
                <span className="font-medium">{hoveredLink.entity}</span>
              </div>
            )}
            {hoveredLink.weight !== undefined && (
              <div className="text-xs opacity-80">
                {t("graph.tooltip.weight")}:{" "}
